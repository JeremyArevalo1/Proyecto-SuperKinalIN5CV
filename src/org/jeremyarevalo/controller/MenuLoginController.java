...
import org.jeremyarevalo.model.Usuario;
...
public class MenuLoginController implements Initializable {
    ...
    public void handleButtonAction(ActionEvent event){
        if(event.getSource() == btnIniciar){
            ...
            if(op == 0){
                if(usuario != null){  
                    if(PasswordUtils.getInstance().checkPassword(pfContrasenia.getText(), usuario.getContrasenia())){
                        SuperKinalAlert.getInstance().alertaSaludo(usuario.getUsuario());
                        if(usuario.getNivelAccesoId() == 1){
                            btnRegistrarU.setDisable(false);
                        ...
                        SuperKinalAlert.getInstance().mostrarAlertaInfo(815);
                    }  
                }else{
                    SuperKinalAlert.getInstance().mostrarAlertaInfo(805);
                }
                }else{
                    stage.menuPrincipalView();
                }
        }else if(event.getSource() == btnRegistrarU){
